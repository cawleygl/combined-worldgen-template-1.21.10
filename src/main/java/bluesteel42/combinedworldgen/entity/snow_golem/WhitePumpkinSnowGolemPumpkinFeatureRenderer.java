...
public class WhitePumpkinSnowGolemPumpkinFeatureRenderer extends FeatureRenderer<WhitePumpkinSnowGolemEntityRenderState, WhitePumpkinSnowGolemEntityModel> {
    private final BlockRenderManager blockRenderManager;

    public WhitePumpkinSnowGolemPumpkinFeatureRenderer(FeatureRendererContext<WhitePumpkinSnowGolemEntityRenderState, WhitePumpkinSnowGolemEntityModel> context, BlockRenderManager blockRenderManager) {
        super(context);
        this.blockRenderManager = blockRenderManager;
    }

    public void render(
            MatrixStack matrixStack, OrderedRenderCommandQueue orderedRenderCommandQueue, int i, WhitePumpkinSnowGolemEntityRenderState snowGolemEntityRenderState, float f, float g
    ) {
        if (snowGolemEntityRenderState.hasPumpkin) {
            if (!snowGolemEntityRenderState.invisible || snowGolemEntityRenderState.hasOutline()) {
                ...
                BlockState blockState = ModPumpkinBlocks.CARVED_WHITE_PUMPKIN.getDefaultState();
                ...
                RenderLayer renderLayer = snowGolemEntityRenderState.hasOutline() && snowGolemEntityRenderState.invisible
                        ...
                        : RenderLayers.getEntityBlockLayer(blockState);
