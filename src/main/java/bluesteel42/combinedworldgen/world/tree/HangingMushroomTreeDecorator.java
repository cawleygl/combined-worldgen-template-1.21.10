...
import bluesteel42.combinedworldgen.block.flora.TrunkAttachedMushroomBlock;
                    ...
                    for (Direction direction : Direction.Type.HORIZONTAL) {
                        if (random.nextFloat() <= 0.1F) {
                            ...
                            if (generator.isAir(blockPos)) {
                                generator.replace(blockPos, ModFloraBlocks.TRUNK_ATTACHED_BROWN_MUSHROOM.getDefaultState().with(TrunkAttachedMushroomBlock.MUSHROOMS, random.nextInt(3) + 1).with(TrunkAttachedMushroomBlock.FACING, direction));
                            }
                        }
                    }
                });
