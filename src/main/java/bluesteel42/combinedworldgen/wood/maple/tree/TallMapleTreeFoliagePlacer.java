...
public class TallMapleTreeFoliagePlacer extends FoliagePlacer {
    public static final MapCodec<TallMapleTreeFoliagePlacer> CODEC = RecordCodecBuilder.mapCodec(instance -> createCodec(instance).apply(instance, TallMapleTreeFoliagePlacer::new));
    protected final int height;
    protected final int mixedLeafChance;

    protected static <P extends TallMapleTreeFoliagePlacer> Products.P4<RecordCodecBuilder.Mu<P>, IntProvider, IntProvider, Integer, Integer> createCodec(RecordCodecBuilder.Instance<P> builder) {
        return fillFoliagePlacerFields(builder)
                .and(Codec.intRange(0, 16).fieldOf("height").forGetter(placer -> placer.height)).and(Codec.intRange(0, 265).fieldOf("mixedLeafChance").forGetter(placer -> placer.mixedLeafChance));
    }

    public TallMapleTreeFoliagePlacer(IntProvider radius, IntProvider offset, int height, int mixedLeafChance) {
        super(radius, offset);
        this.height = height;
    ...
    protected FoliagePlacerType<?> getType() {
        return ModTreeBuilders.TALL_MAPLE_TREE_FOLIAGE_PLACER;
    }
    ...
    protected void generate(
        ...
        for (int i = offset; i >= offset - foliageHeight; i--) {
            int j = (i > 0 && radius > 2) ? 1 : Math.max(radius - 1 - i / 2, 0);
    ...
    protected boolean isInvalidForLeaves(Random random, int dx, int y, int dz, int radius, boolean giantTrunk) {
        ...
        if (radius == 4) {
            ...
            // Remove positions near corners
            if ((dx == radius || dz == radius) && (dx >= radius - 1 && dz >= radius - 1)) {
                return true;
            }
            // Remove next positions with 50% chance
            if ((dx == radius || dz == radius) && (dx >= radius - 2 && dz >= radius - 2)) {
                return random.nextInt(2) == 0;
            }
        }
