...
public class SnowyPineLeavesBlock extends LeavesBlock {
    public static final MapCodec<SnowyPineLeavesBlock> CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance.group(
                    ...
                    .apply(instance, SnowyPineLeavesBlock::new)
    );
    protected final ParticleEffect leafParticleEffect;

    public SnowyPineLeavesBlock(float leafParticleChance, ParticleEffect leafParticleEffect, AbstractBlock.Settings settings) {
    ...
    public static BlockState getMeltedState(BlockState state, WorldAccess world, BlockPos pos) {
        return updateDistanceFromLogs(PineWoodModBlocks.MOD_LEAVES.getDefaultState(), world, pos).with(PERSISTENT, state.get(PERSISTENT)).with(WATERLOGGED, state.get(WATERLOGGED));
    }
    ...
    @Override
    public MapCodec<SnowyPineLeavesBlock> getCodec() {
        return CODEC;
    }
}
