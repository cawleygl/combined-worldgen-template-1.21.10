...
public class CarvedGreenPumpkinBlock extends HorizontalFacingBlock {
    public static final MapCodec<CarvedGreenPumpkinBlock> CODEC = createCodec(CarvedGreenPumpkinBlock::new);
    ...
    private static final Predicate<BlockState> IS_GOLEM_HEAD_PREDICATE = state -> state != null
            && (state.isOf(ModPumpkinBlocks.CARVED_GREEN_PUMPKIN) || state.isOf(ModPumpkinBlocks.GREEN_JACK_O_LANTERN));

    @Override
    public MapCodec<? extends CarvedGreenPumpkinBlock> getCodec() {
        return CODEC;
    }

    public CarvedGreenPumpkinBlock(Settings settings) {
        super(settings);
    ...
    private void trySpawnEntity(World world, BlockPos pos) {
        ...
        if (result != null) {
            GreenPumpkinSnowGolemEntity snowGolemEntity = ModEntities.GREEN_PUMPKIN_SNOW_GOLEM.create(world, SpawnReason.TRIGGERED);
